...
def _intersection_kernel(
    ...
    bound_base = triangle_id * 4
    j0 = tl.load(bounds + bound_base)
    j1 = tl.load(bounds + bound_base + 1)
    k0 = tl.load(bounds + bound_base + 2)
    k1 = tl.load(bounds + bound_base + 3)
    width_k = k1 - k0 + 1
    cell_count = tl.where((j0 <= j1) & (k0 <= k1), (j1 - j0 + 1) * width_k, 0)

    base = triangle_id * 9
    ax = tl.load(triangles + base).to(tl.float64)
    ay = tl.load(triangles + base + 1).to(tl.float64)
    az = tl.load(triangles + base + 2).to(tl.float64)
    bx = tl.load(triangles + base + 3).to(tl.float64)
    by = tl.load(triangles + base + 4).to(tl.float64)
    bz = tl.load(triangles + base + 5).to(tl.float64)
    cx = tl.load(triangles + base + 6).to(tl.float64)
    cy = tl.load(triangles + base + 7).to(tl.float64)
    cz = tl.load(triangles + base + 8).to(tl.float64)
    ...
    while cell_base < cell_count:
        ...
        j = j0 + cell_ids // width_k
        k = k0 + cell_ids % width_k
        py = j.to(tl.float64)
        pz = k.to(tl.float64)
        ya, za = fya - py, fza - pz
        yb, zb = fyb - py, fzb - pz
        ...
        interval = tl.maximum(tl.ceil(intersection).to(tl.int32), 0)
        tl.atomic_add(
            counts + interval * size * size + j * size + k,
            1,
...
def _launch_intersections(
    ...
    kernel(triangles, bounds, counts, size, 256, enable_fp_fusion=False)
...
def apply_signs(
    ...
    _launch_intersections(
        _intersection_kernel[(n_triangles,)],
        triangles,
        bounds,
        counts,
        size,
