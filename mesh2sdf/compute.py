...
def _parse_backend(backend: str) -> Backend:
  match backend:
    case "auto" | "cpu" | "cuda":
      return backend
    case _:
      raise InvalidBackendError(f"Unknown backend: {backend}")


def _cuda_available() -> bool:
  if find_spec("torch") is None or find_spec("triton") is None:
    return False
  from mesh2sdf._cuda import is_available
  return is_available()
...
def _compute_grid(vertices: NDArray[np.generic], faces: NDArray[np.generic],
  ...
  if not use_cuda:
    return mesh2sdf.core.compute(vertices, faces, size)
  from mesh2sdf._cuda import compute_cuda
