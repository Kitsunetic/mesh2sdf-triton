...
def _distance_kernel(
    ...
    for tile in range(0, tl.cdiv(n_triangles, block_triangles)):
        ...
        triangle_grid = tl.broadcast_to(
            triangle_ids[None, :], (block_points, block_triangles)
        )
        close_triangle = tl.where(
            distance <= tile_best[:, None] + tolerance,
            triangle_grid, n_triangles,
        )
        selected_triangle = tl.min(close_triangle, axis=1)
        selected_distance = tl.sum(
            tl.where(triangle_grid == selected_triangle[:, None], distance, 0.0),
            axis=1,
        )
        update = selected_distance < best - tolerance
        best = tl.where(update, selected_distance, best)
        best_triangle = tl.where(update, selected_triangle,
                                 best_triangle)
